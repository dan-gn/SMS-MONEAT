...
filename.append(f'final_results_asc/sms_moneat/results_sms_moneat_final6_full_{metric}2_train.csv')
...
filename.append(f'final_results_asc/mochc/results_mochc_final2024_full_ws_train_{metric}2.csv')
...
color = {
    'loss' : 'lavender',
    'av_loss' : 'lavenderblue',
    'fs' : 'peach',
    'av_fs' : 'deeppeach',
    'gmean' : 'blueblue',
    'av_gmean' : 'columbiablue',
    'hv' : 'deepmossgreen',
    'av_hv' : 'mossgreen',
}
...
with open(f'final_results_asc/tables/{metric}.txt', 'w') as f:
    ...
    for j, ds in enumerate(datasets):
        ...
        # line = f'{ds_name}\t'
        mean_values = [r.mean() for r in row]
        if metric in ['loss', 'fs']:
            best_index = mean_values.index(min(mean_values))
        else:
            best_index = mean_values.index(max(mean_values))
        line = f'{dataset_names[j]}\t'
        for i, r in enumerate(row):
            if i == best_index:
                line += f'\t&\\cellcolor{{{color[metric]}}} {r.mean():.2f} &\\cellcolor{{{color[metric]}}} $\\pm$ {r.std():.1f}'
            else:
                line += f'\t& {r.mean():.2f} &$\\pm$ {r.std():.1f}'
        line += '\\\\ \n'
        f.write(line)
        for i, r in enumerate(row):
            all_rows[i].append(r)
    all_rows = [pd.DataFrame(r).stack() for r in all_rows]
    mean_values = [r.mean() for r in all_rows]
    if metric in ['loss', 'fs']:
        best_index = mean_values.index(min(mean_values))
    else:
        best_index = mean_values.index(max(mean_values))
    ...
    for i, r in enumerate(all_rows):
        if i == best_index:
            line += f'\t&\\cellcolor{{{color[av_metric]}}} {r.mean():.2f} &\\cellcolor{{{color[av_metric]}}} $\\pm$ {r.std():.1f}'
        else:
            line += f'\t& {r.mean():.2f} &$\\pm$ {r.std():.1f}'
    line += '\\\\ \n'
    f.write(line)
