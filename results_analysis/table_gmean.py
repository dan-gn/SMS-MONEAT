...
filename = []
alg = 'n3o'
filename.append(f'final_exp/{alg}/results_{alg}_final2_full_svm_arch.csv')
alg = 'sms_emoa'
filename.append(f'final_exp/{alg}/results_{alg}_final5x_full_svm_train.csv')
alg = 'sms_moneat'
filename.append(f'final_exp/{alg}/results_{alg}_final6_full_svm_val.csv')
filename.append(f'final_exp/{alg}/results_{alg}_final6_full_svm_arch.csv')


df = [pd.read_csv(file) for file in filename]

rm_words = ['Breast_', 'Colorectal_', 'Leukemia_', 'Liver_', 'Prostate_']
with open('table_gmean_svm.txt', 'w') as f:
