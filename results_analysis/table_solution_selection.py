...
filename = 'results_sms-moneat_final6.csv'
cols = [3, 11]
...
with open('table_solutions_sms-moneat_P.txt', 'w') as f:
    all_rows = []
    for ds in datasets:
        row = data[ds]
        ds_name = re.sub(r'|'.join(map(re.escape, rm_words)), '', ds)
        ds_name = re.sub(r'_', '\_', ds_name)
        line = f'{ds_name}\t& {row[0]:.4f} $\pm$& {row[1]:.2f} & {row[2]:.2f} $\pm$& {row[3]:.2f} & {row[4]:.4f} $\pm$& {row[5]:.2f} & {row[6]:.2f} $\pm$& {row[7]:.2f} \\\\ \n'
        f.write(line)
        all_rows.append(row)
